...
import { SCSSHelper } from "@supernovaio/export-utils/helpers/SCSSHelper"
import { GeneralHelper } from "@supernovaio/export-utils/helpers/GeneralHelper"
import { NamingHelper } from "@supernovaio/export-utils/helpers/NamingHelper"
...
export function convertedToken(
  ...
  // Generate the SCSS variable name based on token properties and configuration
  const name = `$${tokenVariableName(token, tokenGroups, collections)}`

  // Convert token value to SCSS, handling references and formatting according to configuration
  const value = SCSSHelper.tokenToSCSS(token, mappedTokens, {
    allowReferences: exportConfiguration.useReferences,
    decimals: exportConfiguration.colorPrecision,
    colorFormat: exportConfiguration.colorFormat,
    forceRemUnit: exportConfiguration.forceRemUnit,
    remBase: exportConfiguration.remBase,
    // Custom handler for token references - converts them to SCSS variable syntax
    tokenToVariableRef: (t) => {
      return `$${tokenVariableName(t, tokenGroups, collections)}`
    },
  })
