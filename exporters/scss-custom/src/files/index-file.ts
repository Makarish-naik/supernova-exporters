 ...
 *    - tokens.dark.scss (dark theme)
 *    - tokens.light.scss (light theme)
 * 
 * 2. Separate by Type: Tokens are split into files by type for each theme
 *    - base/colors.scss, base/typography.scss (base tokens)
 *    - dark/colors.scss, dark/typography.scss (dark theme)
 *    - light/colors.scss, light/typography.scss (light theme)
 * 
...
export function indexOutputFile(tokens: Array<Token>, themes: Array<TokenTheme | string> = []): OutputTextFile | null {
  ...
  if (exportConfiguration.fileStructure === FileStructure.SingleFile) {
    // Generate import for base tokens file (tokens.scss)
    const baseImport = exportConfiguration.exportBaseValues 
      ? `/* Base tokens */\n@import "./tokens.scss";`
      : ''

    // Generate imports for theme files (tokens.{theme}.scss)
    const themeImports = themes.map((theme) => {
      ...
      return `/* Theme: ${themeName} */\n@import "./tokens.${themePath}.scss";`
    }).join("\n\n")

    const separator = baseImport && themeImports ? "\n\n" : ""
    const fileName = FileNameHelper.ensureFileExtension(exportConfiguration.indexFileName, '.scss')
  ...
  // Generate imports for base token files (./base/color.scss, ./base/typography.scss, etc.)
  const imports = exportConfiguration.exportBaseValues 
    ? `/* Base tokens */\n` + types
        .map((type) => `@import "${exportConfiguration.baseStyleFilePath}/${getStyleFileName(type, '.scss')}";`)
        .join("\n")
    : ''
  ...
  const themeImports = themes.map((theme) => {
    ...
    return themeTypes
      .map((type, index) => {
        ...
        return `${themeComment}@import "./${themePath}/${getStyleFileName(type, '.scss')}";`
      })
      .join("\n")
  }).join("\n\n")
  ...
  // Ensure the index file has the correct .scss extension
  const fileName = FileNameHelper.ensureFileExtension(exportConfiguration.indexFileName, '.scss')
